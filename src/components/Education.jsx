...
const FeedbackCard = ({
  ...
  >
    ...
    <div className='mt-1'>
      ...
      <p className='mt-3 text-center green-text-gradient'>{gpa}</p>
      {academicDistinctions.length > 0 && (
        <div className='mt-3 text-center text-academic-distinctions'>
          <p className='text-white font-semibold'>Academic Distinctions:</p>
          {academicDistinctions.map((distinction, i) => (
            <p key={i} className='text-white'>{distinction}</p>
          ))}
        </div>
      )}
    </div>
...
const Education = () => {
  return (
    <div className={`mt-12 bg-black-100 rounded-[20px] text-center`}>
      <div
